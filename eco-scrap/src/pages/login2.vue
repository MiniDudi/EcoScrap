<template>
    <v-container fluid class="pa-0 fill-height login-container">
      <v-row no-gutters class="fill-height">
        <!-- Left side with enhanced eco-themed animated background -->
        ...
        <v-col cols="12" md="5" class="d-flex align-center justify-center login-form-col">
          <v-card class="login-card" elevation="10" max-width="450" width="100%">
            <v-card-text>
              <h1 class="text-h3 font-weight-bold text-center eco-primary--text mb-6">Welcome to EcoScrap</h1>
              <p class="text-subtitle-1 text-center mb-8">Join us in our eco-friendly journey</p>
              ...
              <v-form @submit.prevent="handleLogin">
                ...
                <div class="text-center">
                  <a href="#" class="text-decoration-none eco-link">Forgot password?</a>
                </div>
              </v-form>
            </v-card-text>
          </v-card>
    ...
    methods: {
      async handleLogin() {
        ...
        try {
          ...
          // TODO: Implement error handling (e.g., show snackbar with error message)
        }
      },
      randomLeafStyle() {
        const size = Math.random() * (100 - 50) + 50; // Random size between 50px and 100px
        const left = Math.random() * 100; // Random horizontal position
        const animationDuration = Math.random() * (15 - 10) + 10; // Random duration between 10s and 15s
